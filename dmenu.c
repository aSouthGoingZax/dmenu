...
/* enums */
enum { SchemeNorm, SchemeSel, SchemeOut, SchemeNormHighlight, SchemeSelHighlight, SchemeOutHighlight, SchemeLast, SchemeOpaque }; /* color schemes */
struct item {
	char *text;
...
setup(int z)
{
	...
	win = XCreateWindow(dpy, parentwin, x, y, mw, mh, border_width, depth, InputOutput, visual,
...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
		...
		/* these options take one argument */
			...
			embed = argv[++i];
		else if (!strcmp(argv[i], "-z"))   /* no transparency */
			z = 1;
		else
			usage();
